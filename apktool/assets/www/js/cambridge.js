/**
 * @file
 * The city-specific functions for iReport mobile app.
 *
...
( function ( ) {
  ...
  var $ = xui,
    ...
    /**
     * prod_server: Points to development environment
     * deploy_server: Points to the production environment
     */
    prod_server = "http://devmobile.cambridgema.gov",
    deploy_server = "https://www.cambridgema.gov",

    /**
     * app_location: Where the ticket class files are stored
     */
  ...
  function getReports( ) {
    ...
    server_timeout = setTimeout( abortXHR, server_timeout_limit );
    server_xhr = content.xhr( server + "/" + app_location + "/GetTickets.aspx?deviceID=" + Params.device_id, {
      async: true,
      method: 'get',
      headers: {
  ...
  /**
   * Adds file to the params attachment data
   * @param: {Object} file
   */
  ...
  function mfail( error ) {
    navigator.notification.alert(
      "Unable to retrieve file properties: " + error.code,
      null,
      "Alert",
      "Close"
    );
  ...
  /**
   * Set form values if local storage has them
   */
